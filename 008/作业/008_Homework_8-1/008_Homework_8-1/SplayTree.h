...
BinNodePosi(T) Splay<T>::insert(const T &e)
{
	...
	if(p->_data == e)
	{
		...
		splay(p);
		BinNodePosi(T) newNode = new BinNode<T>(e, NULL, p, p->_rc);
		this->_root = newNode;
		if(p->_rc)
		{
			p->_rc->_parent = newNode;
			p->_rc = NULL;
		}
		p->_parent = newNode;
		this->updateHeight(p);
...
template<typename T>
int Splay<T>::searchAll(const T &e)
{
	BinNodePosi(T) &v = BST<T>::searchIn(this->_root, e, this->_hot = NULL);	
	if(!v)
	{
...
template<typename T>
bool Splay<T>::removeAll(const T &e)
{
	BinNodePosi(T) &v = BST<T>::searchIn(this->_root, e, this->_hot = NULL);
	if(!v)
	{
	...
	else
	{
		...
		splay(p);
		while(this->_root && this->_root->_data == e)
		{
			remove(e);
		}
		return true;
