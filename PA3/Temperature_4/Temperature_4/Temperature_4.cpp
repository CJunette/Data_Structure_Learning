...
void createNewTree(Vector<RangeNode<Station>> *tree, int n)
{
	...
	for(int i = 0; i < n; i++)
	{		
		tree->_elem[start + i]._sizeBelow = 1;
		tree->_elem[start + i]._largestBelow = &tree->_elem[start + i];
		tree->_elem[start + i]._isInternal = true;
	}

	//将范围搜索树最底层空缺节点的isInternal设为false。
	for(int i = start + n; i < size; i++)
	{
		tree->_elem[i]._data = 0;
		tree->_elem[i]._isInternal = false;
	}
	...
	for(int i = start - 1; -1 < i; i--)
	{
		...
		if(!tree->_elem[lChild(i)]._isInternal && !tree->_elem[rChild(i)]._isInternal)
		{
			tree->_elem[i]._data = 0;
			tree->_elem[i]._isInternal = false;
			continue;
		...
		tree->_elem[i]._sizeBelow = tree->_elem[lc]._sizeBelow + tree->_elem[rc]._sizeBelow;
	}
...
{	
	...
	for(int i = 0; i < size; i++)
	{
		...
		{
			associatedTree->_elem[i + associStart]._data._temp = newArr[i]._temp;
			associatedTree->_elem[i + associStart]._data._x = newArr[i]._x;
			associatedTree->_elem[i + associStart]._data._y = newArr[i]._y;
		}
...
long long calAvr(int &x1, int &y1, int &x2, int &y2, int n, int size, Vector<RangeNode<Station>> *mainTree)
{
	...
	{
		int down = searchInAssociatedTree(y1, subTree[i]->_size, subTree[i]);

		int up = searchInAssociatedTree(y2, subTree[i]->_size, subTree[i]);
		if(up == -1)
		{
			up = (size - 1) / 2 + n - 1;
		}
