...
//修改无效。
//取消RangeNode中的_ptr。
//超时了，需要把现在用于排序的array放到原来的_ptr里去。
...
struct RangeNode
{
	...
	RangeNode(T data_ = { 0, 0, 0 }, int sizeBelow_ = 0, RangeNode *parent_ = NULL, RangeNode *lc_ = NULL, RangeNode *rc_ = NULL, RangeNode *largestBelow_ = NULL):
		_data(data_), _sizeBelow(sizeBelow_), _parent(parent_), _lc(lc_), _rc(rc_), _largestBelow(largestBelow_), _succ(NULL) {}
...
long long calAvr(int &x1, int &y1, int &x2, int &y2, RangeTree<station_type> *tree)
{
	...
	*/
	...
	int count = 1;
	RangeNode<station_type> *countPtr = left;
	while(countPtr != right)
	{
		countPtr = countPtr->_succ;
		count++;
	}
	...
	int index = 0;
	RangeNode<station_type> *start = left;
	while(start != right)
	{
		arr[index++] = start;
		start = start->_succ;
	...
	*/
	...
	{
		int down = searchInArray(y1, arr, index);
		int up = searchInArray(y2, arr, index);
		
		if(down == up)
		{
			if(arr[up]->_data.y <= y2 && y1 <= arr[down]->_data.y)
			{
				totalTemp += arr[up]->_data.temp;
				totalStations++;
			}
		}
		else
		{
			...
			while(start < up)
			{
				totalTemp += arr[start]->_data.temp;
				totalStations++;
				start++;
			}
			if(arr[up]->_data.y <= y2)
			{
				totalTemp += arr[up]->_data.temp;
				totalStations++;
			}
