// Temperature_7.cpp : This file contains the 'main' function. Program execution begins and ends there.
//以树的方式从头写。
//缩小rangeNode的大小。
//修改无效。
...
struct RangeNode
{
	...
	RangeNode(T data_ = { 0, 0, 0 }, RangeNode *parent_ = NULL, RangeNode *lc_ = NULL, RangeNode *rc_ = NULL, RangeNode *largestBelow_ = NULL):
		_data(data_), _parent(parent_), _lc(lc_), _rc(rc_), _ptr(NULL) {}
...
RangeNode<station_type> *findLargestBelow(RangeNode<station_type> *node)
{
	if(!node->_lc)
	{
		return node;
	...
	{
		RangeNode<station_type> *largest = node;
...
int findSizeBelow(RangeNode<station_type> *node)
{
	int count = 0;
...
int buildTree(RangeNode<station_type> **arr, int n)
{
	...
	while(1 < size)
	{
		...
		{			
			RangeNode<station_type> *largestLeft = findLargestBelow(arr[i]);

			station_type sta = { largestLeft->_data.x,  largestLeft->_data.y, largestLeft->_data.temp };
			if(i + 1 < size)
			{
				int sizeBelow = findSizeBelow(arr[i]) + findSizeBelow(arr[i + 1]);
				RangeNode<station_type> *largestRight = findLargestBelow(arr[i + 1]);
				RangeNode<station_type> *newNode = new RangeNode<station_type>(sta, NULL, arr[i], arr[i + 1], largestRight);
			...
			{
				int sizeBelow = findSizeBelow(arr[i]);
				RangeNode<station_type> *newNode = new RangeNode<station_type>(sta, NULL, arr[i], NULL, largestLeft);
				newNode->_lc->_parent = newNode;
				arr[i] = newNode;
			}
		}
...
struct AssociatedTree
{
	void operator() (RangeNode<station_type> *node)
	{
		...
		{			
			RangeNode<station_type> *copyNode = que.dequeue();
			station_type sta = { copyNode->_data.x, copyNode->_data.y, copyNode->_data.temp };
			arr[i] = new RangeNode<station_type>(sta);
		}

		heapSort(arr, 0, size, Y);		

		int treeSize = buildTree(arr, size);
...
long long calAvr(int &x1, int &y1, int &x2, int &y2, RangeTree<station_type> *tree)
{
	...
	int count = findSizeBelow(tree->_root);
