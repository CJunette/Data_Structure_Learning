// Temperature_9.cpp : This file contains the 'main' function. Program execution begins and ends there.
...
int searchInArray(int y, RangeNode<station_type> **arr, int size)
{
...
long long calAvr(int &x1, int &y1, int &x2, int &y2, RangeTree<station_type> *tree, double time[7])
{
	...
	timespec_get(&end, 1);
	time[0] += (end.tv_sec - start.tv_sec) + 0.000000001 * (end.tv_nsec - start.tv_nsec);
	...
	timespec_get(&end, 1);
	time[1] += (end.tv_sec - start.tv_sec) + 0.000000001 * (end.tv_nsec - start.tv_nsec);
	...
	RangeNode<station_type> ***arr = new RangeNode<station_type> * *[height];
	if(!arr)
	{
	...
	timespec_get(&end, 1);
	time[2] += (end.tv_sec - start.tv_sec) + 0.000000001 * (end.tv_nsec - start.tv_nsec);
	...
	for(int i = 0; i < index; i++)
	{
		...
		timespec_get(&end, 1);
		time[3] += (end.tv_sec - start.tv_sec) + 0.000000001 * (end.tv_nsec - start.tv_nsec);

		timespec_get(&start, 1);

		if(0 <= down - 1 && y1 <= arr[i][down - 1]->_data.y)
		{
			down--;
		}
		if(down == up)
		...
		timespec_get(&end, 1);
		time[4] += (end.tv_sec - start.tv_sec) + 0.000000001 * (end.tv_nsec - start.tv_nsec);
	}
	...
	timespec_get(&end, 1);
	time[5] += (end.tv_sec - start.tv_sec) + 0.000000001 * (end.tv_nsec - start.tv_nsec);
