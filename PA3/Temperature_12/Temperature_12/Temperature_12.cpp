...
//在RangeNode中添加_lastSame，用以记录最后一个x值相同的节点位置。
//在RangeNode中添加_pred，用以记录上一个节点所在的位置。
...
typedef struct _station_type
{
	int x = 0;
	int y = 0;
	//由于之后AssociatedTree中会用temp作为温度的存储器，因此这里需要将temp设置为long long类型，避免溢出。
	long long temp = 0;
} station_type;
...
struct RangeNode
{
	...
	RangeNode(T data_ = { 0, 0, 0 }, int sizeBelow_ = 0, RangeNode *parent_ = NULL, RangeNode *lc_ = NULL, RangeNode *rc_ = NULL, RangeNode *largestBelow_ = NULL):
		_data(data_), _sizeBelow(sizeBelow_), _parent(parent_), _lc(lc_), _rc(rc_), _largestBelow(largestBelow_), _ptr(NULL), _succ(NULL), _pred(NULL), _lastSame(NULL) {}
...
RangeTree<station_type> *createTree(int n)
{
	...
	if(1 < n)
	{
		...
		{
			int j = i - 1;
			while(0 <= j && arr[i]->_data.x == arr[j]->_data.x)
			{
				arr[j]->_lastSame = arr[i];
				j--;				
			}
			i = j;
		}
...
long long calAvr(int &x1, int &y1, int &x2, int &y2, RangeTree<station_type> *tree)
{
	...
	//当前的right可能大于x2，此时right中的_ptr不会被利用，因此应该让right变为right->_pred，以保证right的_ptr也会被使用到。
	if(left != right && x2 < right->_data.x)
	{
		right = right->_pred;
	}

	//若right所找到的是若干个相同元素的最前侧的元素，则通过_lastSame将right定位到最后侧的元素。
	//如果由于x1取值小于树内所有元素的x值，则left和right会同时取到树中最左侧的值。此时如果不加判断就直接用_lastSame跳转，会导致原来应该不进行判断的x区间被强行拓宽。
	if(right->_data.x <= x2 && right->_lastSame)
	{
		right = right->_lastSame;
	}
	...
	if(!divide->_lc)
	{
		//由于即使只有一个元素，同样可能出现该元素不在[x1, x2]范围内的情况，因此必须进行条件判定。
		if(x1 <= divide->_data.x && divide->_data.x <= x2)
		{
			arrSize[index] = 1;
	...
	else
	{
		...
		while(rDivide->_lc)
		{
			if((rDivide->_data.x < right->_data.x) || (rDivide->_rc && rDivide->_rc->_data.x == right->_data.x))
			{
		...
		*/
		...
		//right似乎需要通过反向的方式来进行搜索。上面的方法有问题，但我实在没想到反例。
		RangeNode<station_type> *rightCheck = right;
		arrSize[index] = 1;
		arr[index++] = right->_ptr;

		while(right != rDivide)
		{
			if(isRChild(right))
			{
				arrSize[index] = right->_parent->_lc->_sizeBelow;
				arr[index++] = right->_parent->_lc->_ptr;
				right = right->_parent;
			}
			else
			{
	...
	for(int i = 0; i < index; i++)
	{
		...
		//由于down是数组中y不大于y1的最大秩，很有可能down所对应的元素的y会小于y1，此时需要将down加一。
		if(down != up && arr[i][down]->y < y1)
		{
			down++;
		}

		//如果存在多个与down对应元素的y相同的元素，则一直向前直到第一个。
		...
		if(down == up)
		{
			if(y1 <= arr[i][down]->y && arr[i][up]->y <= y2)
			{
				if(0 <= down - 1)
				{
					totalTemp += arr[i][up]->temp - arr[i][up - 1]->temp;
				}
				else
				{
		...
		{
			if(y1 <= arr[i][j]->y && arr[i][j]->y <= y2)
			{
				totalTemp += arr[i][j]->temp;
				totalStations++;
			}			
		}
		*/
